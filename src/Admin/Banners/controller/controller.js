...
const addBanner = async (req, res) => {
  upload(req, res, async function (error) {
    ...
    try {
      ...
      const lastProduct = await BannerDataModel.findOne(
        {},
        {},
        { sort: { sort_id: -1 } }
      ).lean();
      ...
      const newBanner = new BannerDataModel({
        ...
        description,
        product_color,
        title_image: req.files["title_image"]?.[0]
          ? `banner_images/${req.files["title_image"][0].filename}`
          : "",
        thumbnail_image: req.files["thumbnail_image"]?.[0]
          ? `banner_images/${req.files["thumbnail_image"][0].filename}`
          : "",
        banner_image: req.files["banner_image"]?.[0]
          ? `banner_images/${req.files["banner_image"][0].filename}`
          : "",
      });
...
const updateBanner = async (req, res) => {
  upload(req, res, async function (error) {
    ...
    try {
      const { banner_id, title, description, product_color,sort_id } = req.body;
      ...
      banner.sort_id = sort_id || banner.sort_id;
      banner.title = title || banner.title;
      banner.description = description || banner.description;
      banner.product_color = product_color || banner.product_color;
      banner.title_image = req.files["title_image"]?.[0]
        ? `banner_images/${req.files["title_image"][0].filename}`
        : banner.title_image;
      banner.thumbnail_image = req.files["thumbnail_image"]?.[0]
        ? `banner_images/${req.files["thumbnail_image"][0].filename}`
        : banner.thumbnail_image;
      banner.banner_image = req.files["banner_image"]?.[0]
        ? `banner_images/${req.files["banner_image"][0].filename}`
        : banner.banner_image;
